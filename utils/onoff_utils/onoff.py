from utils.onoff_utils import onoff_opt, onoff_llama, onoff_qwen3
...
def block_replace(model):
    if 'opt' in model.name.lower():
        ...
        model = onoff_qwen3.block_replace(model)
    return model
...
def turn_off(model, block_idx):
    if 'opt' in model.name.lower():
        ...
        onoff_qwen3.turn_off(model, block_idx)
...
def turn_on(model, block_idx):
    if 'opt' in model.name.lower():
        ...
        onoff_qwen3.turn_on(model, block_idx)
...
def scan(model, num_blocks):
    ...
    if 'opt' in model.name.lower():
        ...
        onoff_qwen3.scan(model, num_blocks)
